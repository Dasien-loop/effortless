...
public class MinecraftContentFactory implements ContentFactory {
    ...
    public ResourceLocation newResourceLocation(String namespace, String path) {
        return new MinecraftResourceLocation(net.minecraft.resources.Identifier.fromNamespaceAndPath(namespace, path));
    }

    @Override
    public Optional<Item> newOptionalItem(ResourceLocation location) {
        return BuiltInRegistries.ITEM.getOptional(location.<net.minecraft.resources.Identifier>reference()).map(MinecraftItem::ofNullable);
    }
