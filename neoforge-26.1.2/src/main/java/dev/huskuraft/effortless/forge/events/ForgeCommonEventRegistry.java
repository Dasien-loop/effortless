...
public class ForgeCommonEventRegistry extends CommonEventRegistry {
    ...
    public void onPlayerChangedDimension(PlayerEvent.PlayerChangedDimensionEvent event) {
        if (event.getEntity() instanceof ServerPlayer player) {
            ...
            getPlayerChangeWorldEvent().invoker().onPlayerChangeWorld(
                    MinecraftPlayer.ofNullable(player),
                    MinecraftWorld.ofNullable(server.getLevel(event.getFrom())),
                    MinecraftWorld.ofNullable(server.getLevel(event.getTo()))
            );
        }
    }
